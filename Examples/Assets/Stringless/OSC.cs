// OSC plugin adapated from Thomas Fredericks UnityOSC
// https://github.com/thomasfredericks/UnityOSC
// 
// This file supports the Remote Control for Unity v2 (Stringless), 
// which was developed with Jorge Garcia UnityOSC,
// due to performance issues, Stringless now incorporates UnityOSC from Thomas Fredericks
// 
// Most of the code was based on UnityOSC 1.01
//
// OSC plugin v2.0 (2016.04.18)
...
/// \mainpage
/// \section Overview
/// This plugin was developed by Thomas Fredericks to support OSC communications inside Unity
/// It was adapted by Luis Grifu for the Stringless plugin
/// You can still use the OSC plugin with coding
/// The description below is adapted from the original text for Make Controller Kit retrieved from UnityOSC by Thomas Fredericks
...
/// Common OSC types thatare integers, floats, and strings.   
...
//{
  ...
  public class OSC : MonoBehaviour
  {
	...
	void OnValidate() {
		...
		if (instancesOSC.Length > 1)
		{
			
			foreach(OSC item in instancesOSC)
			{
				
				if(listenPort == item.getPortIN() && item != this) 
				{
					Debug.Log ("Port already exists! please insert a different port number");
					listenPort = 0;

				}
				if(broadcastPort == item.getPortOUT() && item != this) 
				{
					if(broadcastHost == item.getIP() && item != this)
					{
						Debug.Log ("Port already exists! please insert a different port number");
						broadcastPort = 0;
					}
    ...
    */
	...
	private void setOsc(OscMessage oscMessage)
	{
		oscMessageExpose = oscMessage;
		if (AllMessageSendingHandler != null)
		{
			AllMessageSendingHandler(oscMessageExpose);
		}

	}
  
	public OscMessage getOsc()
	{
		return oscMessageExpose;
	}
	...
	public void setSendMessage(OscMessageHandler amh)
	{
		AllMessageSendingHandler = amh;
	}
